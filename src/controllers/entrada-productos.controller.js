...
import { conExcelLock } from "../utils/excelHelper.js";
...
export const sincronizarEntradaProductos = async (req, res) => {
  try {
    ...
    const resultado = await conExcelLock(async () => {
      ...
      return { productosSincronizados, detalles };
    });

    console.log(`✅ [entrada-productos] ${resultado.productosSincronizados} productos sincronizados al Excel`);

    return res.json({
      success: true,
      sincronizadas: resultado.productosSincronizados,
      sincronizados: resultado.productosSincronizados,
      mensaje: `${resultado.productosSincronizados} productos sincronizados correctamente`,
